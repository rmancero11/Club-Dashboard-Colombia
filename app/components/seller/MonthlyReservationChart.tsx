...
export default function MonthlyReservationsChart({
  ...
  // Dimensiones/estilos coherentes
  const {
    chartHeight,
    margin,
    xTickAngle,
  ...
  const data = useMemo(
    () =>
      safeLabels.map((ym, i) => ({
        ...
        Reservas: Number.isFinite(safeValues[i]) ? (safeValues[i] ?? 0) : 0,
      })),
    [safeLabels, safeValues]
  );

  const hasData = data.some(d => (d.Reservas ?? 0) > 0);

  return (
    <div className="rounded-xl border bg-white p-4" role="group" aria-label="Gráfico de barras de reservas por mes">
          ...
          <div className="w-full" style={{ height: chartHeight }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={margin} barCategoryGap={barCategoryGap} barGap={barGap}>
                ...
                />
                <Tooltip
                  formatter={(value: any) => [value as number, 'Reservas']}
                ...
                <Bar dataKey="Reservas" name="Reservas" fill="#3B82F6" radius={[6, 6, 0, 0]} maxBarSize={barSize} />
              </BarChart>
            </ResponsiveContainer>
          </div>
