...
type MostFrequentDinnersOnTableProps = {
  mostFrequentNumberOfDinnersOnTable: string | null;
  preset: Preset | undefined
  dateRange: DateRange
};

const MostFrequentDinnersOnTable = ({
  mostFrequentNumberOfDinnersOnTable,
  preset,
  dateRange,
  className
}: MostFrequentDinnersOnTableProps & HTMLAttributes<HTMLDivElement>) => {
          ...
          {mostFrequentNumberOfDinnersOnTable && (
            <div>
              <h2 className='text-center text-black font-medium text-lg'>
                {`Personas sentadas en la mesa (${subtitle})`}
              </h2>
              <div className='flex justify-center items-center'>
                <Image
                  className='object-cover relative'
                  src='/people-table.webp'
                  alt='Personas en la mesa'
                  width={315}
                  height={315}
                />
                <p className='text-green-500 text-7xl font-bold absolute'>
                  {mostFrequentNumberOfDinnersOnTable}
                </p>
              </div>
            </div>
...
export default MostFrequentDinnersOnTable
