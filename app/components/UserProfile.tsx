...
export default function UserProfile({ user }: { user: UserShape }) {
  ...
  <div className="flex-1 max-w-sm">
    ...
    {user.vendedor ? (
      <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center gap-4">
        ...
        <p className="text-gray-700 font-semibold text-lg text-center">{user.vendedor.nombre}</p>
        <div className="flex flex-col gap-2 items-center w-full">
          ...
          {user.vendedor.currentlyLink && (
            ...
            >
              <Image src="/favicon/calendly.svg" alt="Calendly" width={16} height={16} />
              Calendly
            </a>
          )}
        </div>
  ...
  <div className="flex-1 max-w-sm">
    ...
    <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center justify-center h-full">
      {/* Condicional: todavía no ha reservado */}
      <p className="text-gray-500 text-center">
        Todavía no has reservado un destino.
      </p>

      {/* Ejemplo de carta de destino cuando haya reserva */}
      {/*
      <div className="rounded-2xl overflow-hidden shadow-md w-full">
        <div className="relative w-full h-48">
          <Image src={destino.imageUrl} alt={destino.name} fill className="object-cover" />
        </div>
        <div className="p-4">
          <h3 className="text-lg font-semibold">{destino.name}</h3>
          <p className="text-sm text-gray-500">{destino.city}, {destino.country}</p>
        </div>
      </div>
      */}
    </div>
