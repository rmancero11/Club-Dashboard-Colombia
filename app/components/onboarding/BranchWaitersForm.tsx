...
type IBrancheWaitersForm = {
  ...
  waiters: Waiters[];
  waitersData: Waiters[];
  removeWaiter: UseFieldArrayRemove;
  ...
  return (
    <Accordion type="multiple">
      {waiters.map((waiter, index) => {
        const waiterData = waitersData[index];
        const waiterId = waiterData.waiterId;
        const isCreatedWaiter = waiterId.length > 0;

        return (
          <AccordionItem value={waiter.id} key={waiter.id} data-state="open">
            <AccordionTrigger>
          ...
          <Button
              ...
              size='sm'
              onClick={() => {
                navigator.clipboard.writeText(`${FEEDBACK_BASE_URL}${businessId}&mesero=${waiterId}`)
                toast({
          ...
          >
            ...
            <AccordionContent>
              <div className="space-y-4 mx-1 md:space-y-0 md:flex items-end md:space-x-8">
                <FormField
                  control={control}
                  name={`waiters.${index}.name` as const}
                  render={({ field }) => (
                    <FormItem>
                ...
                <FormField
                  control={control}
                  name={`waiters.${index}.gender` as const}
                  render={({ field }) => (
                    <FormItem>
                ...
                <Button
                  onClick={async () => {
                    await handleDeleteWaiter({ businessId, waiterId });
                    removeWaiter(index);
                  }}
                  type="button"
                  variant="destructive">
