...
const getTop3Waiters = (waiters: Waiter[]) => {
  const sortedWaiters = waiters.slice().sort((a, b) => {
    if (a.ratingAverage && b.ratingAverage) {
      return (b.numberOfSurveys * b.ratingAverage) - (a.numberOfSurveys * a.ratingAverage)
    }
    return 0
  });
...
type Variant = 'gold' | 'silver' | 'bronze' | 'default' | 'secondary' | null | undefined;
...
function Top3Waiters({ waiters, businessName }: Top3WaitersProps) {
  ...
  return (
    <div className='my-6'>
    <h2 className='text-xl font-bold mb-8'>Top 3 meseros de todas las sucursales de <span className='text-primary' >{businessName}</span></h2>
      ...
      {top3Waiters.map((waiter, idx) => (
        <div key={waiter.name} className={cn('md:col-span-1')}>
          <CardWidget
            ...
            variant={topWaitersVariants[idx as keyof typeof topWaitersVariants]}
            className='relative'
          >
            <Image
            ...
            <div className='flex items-center justify-between'>
              <div className='flex justify-center relative items-center mt-3'>
                <Image
                  ...
                  height={110}
                />
                <div className='flex flex-col bg-qik justify-center items-center absolute top-[calc(100%-30px)] md:top-[calc(100%-30px)]
                rounded-full w-10 py-1'
                >
                  <FontAwesomeIcon icon={faStar} color='#f6d91e' className='w-4 h-4' />
