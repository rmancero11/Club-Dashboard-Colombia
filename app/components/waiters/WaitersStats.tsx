...
const WaitersStats = ({ waiters }: IWaiterProps) => {
  return (
    <div className="flex flex-no-wrap overflow-x-auto scrolling-touch snap-proximity items-start mb-8 pt-6 space-x-8">
      {waiters
        .sort((a, b) => (b.numberOfSurveys ?? 0) - (a.numberOfSurveys ?? 0))
        .map((waiter, idx) => (
          <div
            key={idx}
            className="flex justify-center items-center flex-col rounded-xl min-w-fit border-[1px] border-qik py-3 px-6 mb-4 relative"
          >
            {[0, 1, 2].includes(idx) && (
              <Image
            ...
            <div className="flex justify-center relative items-center mt-3">
              ...
              />
              <div
                className="flex flex-col bg-qik justify-center items-center absolute lg:mt-40 mt-28
