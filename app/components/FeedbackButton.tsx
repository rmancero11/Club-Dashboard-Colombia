...
import {
  useGetBusinessSucursales,
  useGetFeedbackData,
} from "../hooks/business";
import { useState } from "react";
import {
  useGetWaitersByBusiness,
  useGetWaitersBySucursales,
} from "../hooks/waiters";
import {
...
function FeedbackButton() {
  ...
  const { Name } = (business as Business) || {};
                ...
                <DropdownMenuSubContent>
                  {sucursales.map((sucursal, index) => (
                    <DropdownMenuItem key={index}>
                      <a
                        href={generateRestaurantSucursalURL(
                          businessId,
                          sucursal?.id
                        )}
                        target="_blank"
        ...
        {Object.keys(waitersSucursales || {})?.length > 0 && (
          <DropdownMenuGroup>
            <DropdownMenuSub>
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent>
                  {Object.values(waitersSucursales || [])
                    ?.flatMap((item) => item)
                    ?.map((waiter, index) => (
                      <DropdownMenuItem key={index}>
                        <a
                          href={generateWaiterBySucursalURL(
                            businessId,
                            waiter?.sucursalId,
                            waiter?.id
                          )}
