...
export const DateRangePicker: FC<DateRangePickerProps> & {
  filePath: string } = ({
    ...
    const getPresetRange = (presetName: string): DateRange => {
      const preset = PRESETS_TO_USE.find(({ name }) => name === presetName)
      ...
      switch (preset.name) {
        ...
        case 'lastMonth':
          ...
          break
        case 'all':
          from.setTime(creationDate.getTime())
          from.setHours(0, 0, 0, 0)
          to.setHours(23, 59, 59, 999)
          break
      }
    ...
    const checkPreset = (): void => {
      for (const preset of PRESETS_TO_USE) {
    ...
    // adapt range.to to end of day if it is not undefined
    useEffect(() => {
      if (range.to) {
    ...
    return (
      <Popover
        modal open={isOpen} onOpenChange={(open: boolean) => {
          if (!open) {
            resetValues()
          }
          setIsOpen(open)
      ...
      >
        ...
        <PopoverContent align={align} className='w-auto'>
          <div className='flex py-2'>
            ...
            {
            !isSmallScreen && (
              <div className='flex flex-col items-end gap-1 pr-2 pl-6 lg:h-[50vh] overflow-y-scroll'>
          ...
          <div className='flex justify-end gap-2 py-2 pr-4'>
            <Button
              ...
              variant='ghost'
            >
              {isDsc ? 'Cancel' : 'Cancelar'}
            </Button>
            <Button
              disabled={withoutToDate}
              onClick={() => {
                setIsOpen(false)
                if (
