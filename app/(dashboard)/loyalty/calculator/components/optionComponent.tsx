...
const OptionSelector: React.FC<OptionSelectorProps> = ({ selectedOption, onSelect }) => (
  <div className="flex flex-wrap space-x-2 justify-center my-3">
    ...
    {options.map((option, index) => (
      <Button
        className={`rounded-full ${selectedOption === index ? 'bg-primary text-white' : 'bg-white text-primary border border-primary hover:bg-primary hover:text-white'} font-bold py-2 px-4 rounded w-[6rem]`}
        key={index}
