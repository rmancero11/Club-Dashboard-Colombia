...
export default async function AdminUserDetailPage({ params }: { params: { id: string } }) {
  ...
  const user = await prisma.user.findFirst({
    ...
    select: {
      ...
      role: true, status: true, commissionRate: true, createdAt: true,
    },
  });
  if (!user) notFound();
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-white p-4">
          ...
          <div className="grid gap-2 text-sm">
            <div><span className="text-gray-500">Nombre: </span>{user.name || "—"}</div>
            <div><span className="text-gray-500">Email: </span>{user.email}</div>
            <div><span className="text-gray-500">Teléfono: </span>{user.phone || "—"}</div>
            <div><span className="text-gray-500">País: </span>{user.country || "—"}</div>
            <div><span className="text-gray-500">Creado: </span>{new Date(user.createdAt).toLocaleString("es-CO")}</div>
          </div>
        </div>
